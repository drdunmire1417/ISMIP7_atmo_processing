...
class GradientRegridder:
    def __init__(self, config, var):
        ...
        with open(f'attrs/{self.config.method}.json', 'r') as f:
            ...
            self.grad_file = glob(f'{self.config.grad_dir}{grad_file}*')

            if len(self.grad_file)!=1: raise ValueError(f'ERROR: Check gradient folder. No files found for {grad_file}')
